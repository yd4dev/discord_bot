		...
		if (result.welcome_plugin) {
			...
			if (!member.guild.channels.cache.find(c => c.id === result.welcome_channel)) {
				...
				try { member.guild.channels.cache.find(c => c.id === result.welcome_channel).send(Embed); }
					...
					member.guild.channels.cache.some(c => c.type === 'text' && c.permissionsFor(member.guild.members.cache.find(m => m.id === client.user.id)).has('SEND_MESSAGES')).send(`Could not send welcome message into ${member.guild.channels.cache.find(c => c.id === result.welcome_channel)}`);
				}
			}
		}
	});
