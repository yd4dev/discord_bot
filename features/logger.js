...
module.exports = client => {
    ...
    client.on('guildMemberRemove', async member => {
        ...
        const kicked = (await guild.fetchAuditLogs({ limit: 1, type: 'MEMBER_KICK' })).entries.first()
    ...
    client.on('messageDelete', async message => {
        ...
        if (fetchedLogs && fetchedLogs.target.id === message.author.id) Embed.addField('Moderator:', log.executor)
