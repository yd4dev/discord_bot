			...
			if (args[1] === 'create') {
				...
				case 'edit':
					...
					case 'name': {
						...
						const curResult = await client.schemas.get('ticketCategory').findOne({
							guild_id: message.guild.id,
						...
						const newNameCategory = await client.schemas.get('ticketCategory').findOne({
							guild_id: message.guild.id,
				...
				case 'react': {
					...
					const category_channel = message.guild.channels.cache.get(args[3])

					const category_message = await category_channel.messages.fetch(args[4])
